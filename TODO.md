...
- **`AdService.submit()`'s wallet-balance check is a point-in-time gate, not a reservation.** It confirms the advertiser's current balance covers `budgetTotalCoins` at the moment of submission, but does not lock/reserve those coins. An advertiser can submit multiple ads whose combined budgets exceed their actual balance (each `submit()` call checks independently). This is intentional scope for phase 4 (see `docs/DECISIONS.md` ADR-010) - real budget deduction happens per-impression in phase 5's `AdServingService`, which doesn't exist yet. **Action needed before phase 5 is considered done:** design and implement proper budget reservation/locking (or an equivalent safeguard) so total reserved+spent budget across an advertiser's ads can never exceed their wallet balance - this is financial logic per CLAUDE.md and needs explicit design review, not just an incidental fix.
