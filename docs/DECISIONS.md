...
## ADR-014: minimum `cpmCoins` of 1000 (no sub-coin impression cost)

**Date:** 2026-08-27
**Phase:** 5 (found while building `AdServingService`)

`docs/ARCHITECTURE.md` section 4 step 7 says each impression costs `cpmCoins/1000`. Coins are stored as `BigInt` - always integers, per the doc's own opening note - so with `cpmCoins < 1000` (e.g. the `cpmCoins: 500` used in this session's own earlier phase-4 manual tests), integer division truncates the cost to `0`: every impression of that ad would be entirely free, silently leaking the advertiser's intended spend. Nothing in PRD/ARCHITECTURE anticipated this. Presented the user three options (round to nearest coin with a floor of 1; enforce a hard minimum CPM of 1000; track a fractional-coin remainder in a new column) - **user picked the hard minimum**, as the simplest fix with no schema change and no rounding-drift accounting to reason about later.

- `packages/shared-types/src/ad.ts`'s `createAdRequestSchema.cpmCoins` changed from `z.coerce.number().int().positive()` to `.int().min(1000)` - enforced server-side (the same schema `AdController`/`AdService` already validate against) and mirrored client-side in `apps/miniapp/src/pages/AdWizard.tsx`'s CPM input (`min={1000}`, submit button disabled below it, updated helper text).
- No migration needed - this is a request-validation change, not a schema change. Existing `Ad` rows (there are none from real users yet) aren't affected either way.
- `AdServingService.serveAd()` relies on this invariant directly: `costCoins = winner.cpmCoins / 1000n` with a code comment noting it's only safe because creation-time validation guarantees `cpmCoins >= 1000`.

## ADR-015: `GET /serve/ad` / `POST /serve/click` protected by a shared internal token, not per-publisher auth

**Date:** 2026-08-27
**Phase:** 5

`docs/ARCHITECTURE.md` section 5 labels these "داخلی برای موتور نمایش (فراخوانی از bot/SDK)" but never specifies how they're authenticated - and unlike `GET /categories` (harmless reference data, left public), these two directly move real budget and credit real publisher wallets, so leaving them open would be a genuine abuse vector reachable by anyone who found the URL and a `channelId`. Real per-publisher API keys are naturally phase 7's job (channel registration/ownership verification doesn't exist yet), so building that properly now would mean front-running phase 7's design. Presented the user three options (shared internal secret; no auth with a TODO; build the full per-channel key system now) - **user picked the shared internal secret**, matching the "internal, bot/SDK-only" framing in ARCHITECTURE.md for this phase.

- New `INTERNAL_SERVICE_TOKEN` env var (`packages/config`, required - same seriousness as `JWT_SECRET`). Only `apps/bot` (once it actually calls these endpoints - not yet wired, that's a separate `apps/bot` integration task) and this platform's own trusted callers should ever hold it.
- `InternalServiceGuard` (`apps/api/src/serve/internal-service.guard.ts`) checks the `X-Internal-Token` header with `crypto.timingSafeEqual`, same reasoning as `telegram-init-data.ts`'s hash check - a constant-time compare costs nothing and closes a timing side-channel on a money-moving endpoint.
- Added basic rate limiting (`docs/ROADMAP.md` phase 5's "سیستم پایه ضدتقلب" item) via `@nestjs/throttler`, scoped to just `ServeController` (not a global `APP_GUARD`, to avoid silently changing behavior for every other endpoint without separate review): 30 req/10s for `GET /serve/ad`, 10 req/10s for `POST /serve/click`. Verified for real: 15 rapid real HTTP calls to `/serve/click` returned 404 (no such impression) for the first 10, then 429 for the rest.
- **Explicitly out of scope for this phase, tracked in `TODO.md`:** real per-publisher API keys (phase 7), and country-based targeting exclusion (`excludedCountries` in `docs/ARCHITECTURE.md` section 4 step 3) - the viewer's country isn't available anywhere in the current call chain (`apps/bot` doesn't have it either), so the filter is a documented no-op for now rather than guessed at.
- **Verified against the real dev API + dev Postgres end-to-end**, including cross-module data created directly (no `POST /channels` exists yet - phase 7 - so a `Channel` row was inserted directly via Prisma for the test, exactly as ADR-013's verification created test `User`/`Wallet` rows directly): the guard rejected no-token and wrong-token requests with 401 and accepted the real token; three real `GET /serve/ad` calls against a real `ACTIVE` ad (budget 15, CPM 5000 → cost 5/impression) correctly deducted `budgetSpentCoins` to exactly 15, credited the publisher's real wallet with 4 coins each time (80% of 5, `platformCommissionPercent=20`), and flipped the ad to `OUT_OF_BUDGET` after the third impression so a fourth call correctly returned nulls; `POST /serve/click` recorded a real `AdClick`, was idempotent on a repeat call, and 404'd for a bogus `impressionId`; `GET /ads/:id/stats` (phase 4's endpoint) then correctly reported `impressions: 3, clicks: 1, ctr: 0.333, budgetSpentCoins: "15"` from the same real data. 20 unit tests for `ServeService` (97.6% statement coverage).
